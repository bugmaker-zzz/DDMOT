# -*-coding:utf-8-*
import numpy as np
from tracking.cost_function import iou_2d, giou_2d, sdiou_2d, diou_2d, giou_3d, dist_3d, iou_extended, dist_3d_new, iou_dist_2d, gdiou_3d, diou_3d
# from tracking.cost_function import iou_3d
...
def cost_calculate(dets, trks, iou_matrix, iou_threshold, cost_func):
    for d, det in enumerate(dets):
        for t, trk in enumerate(trks):
            ...
            # elif cost_func == 'giou_3d':
            #     iou_matrix[d, t] = giou_3d(det, trk)
            # elif cost_func == 'iou_3d':
...
# new
def associate_dets_to_trks_by_extended_bbox_2d(dets, trks, cost_func, iou_threshold, metric):
    if (len(trks) == 0):
    ...
    if metric == 'match_3d':
        ...
        dets = np.array([d.to_x1y1x2y2_extended() for d in dets])
        trks = np.array([t.to_x1y1x2y2() for t in trks])
        for d, det in enumerate(dets):
            for t, trk in enumerate(trks):
                ext_w = np.minimum(6, (det[4] + 1) * 4.5)
                ext_h = np.minimum(2, (det[4] + 1) * 1.5)
                iou_matrix[d, t] = iou_extended(det[:4], trk, ext_w, ext_h)
        if min(iou_matrix.shape) > 0:
            a = (iou_matrix > iou_threshold).astype(np.int32)
            if a.sum(1).max() == 1 and a.sum(0).max() == 1:
                matched_indices = np.stack(np.where(a), axis=1)
...
def associate_dets_to_trks_fusion(dets, trks, cost_func, iou_threshold, metric):
    ...
    if (len(dets) == 0):
        return np.empty((0, 2), dtype=int), [], np.arange(len(trks))
    # trks = sorted(trks, key=lambda track:track.age, reverse=True)
    iou_matrix = np.zeros((len(dets), len(trks)), dtype=np.float32)
    for d, det in enumerate(dets):
        for t, trk in enumerate(trks):
            if cost_func == 'iou_3d':
                iou_matrix[d, t] = giou_3d(det, trk)
                if iou_matrix[d, t] == 0:
                    iou_matrix[d, t] = dist_3d_new(det, trk)
            elif cost_func == 'giou_3d':
                iou_matrix[d, t] = giou_3d(det, trk)
            elif cost_func == 'dist_3d':
                iou_matrix[d, t] = dist_3d(det, trk)
            elif cost_func == 'gdiou_3d':
                iou_matrix[d, t] = gdiou_3d(det, trk)
            elif cost_func == 'diou_3d':
                iou_matrix[d, t] = diou_3d(det, trk)
    if min(iou_matrix.shape) > 0:
        a = (iou_matrix > iou_threshold).astype(np.int32)
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            matched_indices = np.stack(np.where(a), axis=1)
...
def associate_dets_to_trks(dets, trks, cost_func, iou_threshold, metric):
    if (len(trks) == 0):
        return np.empty((0, 2), dtype=int), np.arange(len(dets)), []
    if (len(dets) == 0):
        return np.empty((0, 2), dtype=int), [], np.arange(len(trks))
...
def trackfusion2Dand3D(trks_2d, trks_3Dto2D_image, iou_threshold):
    ...
    for t, trk in enumerate(trks_2d):
        for d, det in enumerate(trks_3Dto2D_image):
            ...
            # iou_matrix[t, d] = iou_dist_2d(trk.to_x1y1x2y2(), det)  # det: 8 x 3, trk: 8 x 3
    if min(iou_matrix.shape) > 0:
        a = (iou_matrix > iou_threshold).astype(np.int32)
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:
            matched_indices = np.stack(np.where(a), axis=1)

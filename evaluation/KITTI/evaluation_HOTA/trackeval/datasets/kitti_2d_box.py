...
class Kitti2DBox(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        # every time want to experiment new parameter need to change 'TRACKERS_TO_EVAL', 'SPLIT_TO_EVAL'(include kitti.yaml),
        # 'TRACKER_SUB_FOLDER', 'OUTPUT_SUB_FOLDER' and if want experiment ped, need to change 'CLASSES_TO_EVAL'
        default_config = {
            'GT_FOLDER': os.path.join(code_path, '../data/gt'),  # Location of GT data
            'TRACKERS_FOLDER': './results/kitti/training/tracker/category_combined_data',  # Trackers location
            # 'TRACKERS_FOLDER': res_path,  # Trackers location
            'OUTPUT_FOLDER': './results/kitti/training',  # Where to save eval results (if None, same as TRACKERS_FOLDER)
            # 'TRACKERS_TO_EVAL': None,  # Filenames of trackers to eval (if None, all in folder)
            'TRACKERS_TO_EVAL': ['tracking_115'],
            # 'CLASSES_TO_EVAL': ['car', 'pedestrian'],  # Valid: ['car', 'pedestrian']
            'CLASSES_TO_EVAL': ['car'],
            'SPLIT_TO_EVAL': 'training',  # Valid: 'training', 'val', 'training_minus_val', 'test', 'fortest', 'val_new', 'training_new'
            'INPUT_AS_ZIP': False,  # Whethe7 tracker input files are zipped
            'PRINT_CONFIG': True,  # Whether to print current config
            'TRACKER_SUB_FOLDER': 'tracking_115',  # Tracker files are in TRACKER_FOLDER/tracker_name/TRACKER_SUB_FOLDER
            'OUTPUT_SUB_FOLDER': 'eval_result_training_kittipt_yolo_3d_2d_adaptive_gdiou_0.5_0.5',  # Output files are saved in OUTPUT_FOLDER/tracker_name/OUTPUT_SUB_FOLDER

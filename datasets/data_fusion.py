import numpy as np
from tracking.cost_function import iou_2d, convert_3dbox_to_8corner
from tracking.matching import linear_assignment, greedy_match
...
def data_fusion(detections_3D_camera, detection_2D, detection_3Dto2D, additional_info):
    ...
    for d1, det_2D in enumerate(detection_2D):
        for d2, det_3Dto2D in enumerate(additional_info):
            iou_matrix[d1, d2] = iou_2d(det_2D, det_3Dto2D[2:6])  # det: 8 x 3, trk: 8 x 3
...
def data_fusion_new(detections_3D_camera, detection_2D, detection_3Dto2D, additional_info, additional_info_2d):
    '''
    :param detections_3D_camera:  Detections by LiDAR
    :param detection_2D:          Detections by Camera
    :param detection_3Dto2D:      Detections by LiDAR
    :return:
        ...
        detection_3D_only: Objects detected by LiDAR only.
    '''
    # change 0.3 -> 0.5(good), 0.5 -> 0.6(bad), 0.5 -> 0.8(bad)
    iou_threshold = 0.5
    iou_matrix = np.zeros((len(detection_2D), len(detection_3Dto2D)), dtype=np.float32)
    for d1, det_2D in enumerate(detection_2D):
        for d2, det_3Dto2D in enumerate(additional_info):
    ...
    matched, unmatched_detections_2D, unmatched_detection_3Dto2D = [], [], []
    # detection_3D_fusion_info, detection_3D_only_info = [],[]
    # new add (detection_2D_fusion_info not to be used)
    detection_3D_fusion_info, detection_2D_fusion_info, detection_3D_only_info, detection_2D_only_info = [],[],[],[]
    for d, det in enumerate(detection_2D):
        if d not in matched_indices[:, 0]:
            unmatched_detections_2D.append(d)
    ...
    for detection_2D_idx, detection_3Dto2D_idx in matched:
        # new add
        detection_2D_fusion_info.append(additional_info_2d[detection_2D_idx])
        detection_2D_fusion.append(detection_2D[detection_2D_idx].tolist())
        detection_3D_fusion_info.append(additional_info[detection_3Dto2D_idx])
        detection_3Dto2D_fusion.append(detection_3Dto2D[detection_3Dto2D_idx].tolist())
        detection_3D_fusion.append(detections_3D_camera[detection_3Dto2D_idx].tolist())
    ...
    for unmatched_detections_2D_idx in unmatched_detections_2D:
        detection_2D_only.append(detection_2D[unmatched_detections_2D_idx].tolist())
        # new add
        detection_2D_only_info.append(additional_info_2d[unmatched_detections_2D_idx])
    ...
    detection_3D_only = {'dets_3d_only': detection_3D_only, 'dets_3d_only_info': detection_3D_only_info}
    # new add
    detection_fusion_2D_3D = {'dets_3d_fusion': detection_3D_fusion, 'dets_3d_fusion_info': detection_3D_fusion_info, 'dets_2d_fusion': detection_2D_fusion, 'dets_2d_fusion_info': detection_2D_fusion_info}
    detection_2D_only = {'dets_2d_only': detection_2D_only, 'dets_2d_only_info': detection_2D_only_info}

    # return detection_3D_fusion, detection_3D_only, detection_2D_only
    return detection_fusion_2D_3D, detection_3D_only, detection_2D_only
